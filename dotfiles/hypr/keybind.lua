...
-- Brightness (repeat)
hl.bind("XF86MonBrightnessDown", hl.dsp.exec_cmd(SCRIPT .. "/brightness --dec"), { repeating = true })
hl.bind("XF86MonBrightnessUp",   hl.dsp.exec_cmd(SCRIPT .. "/brightness --inc"), { repeating = true })

-- Volume (locked + repeat)
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd(SCRIPT .. "/volume --inc"),  { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd(SCRIPT .. "/volume --dec"),  { locked = true, repeating = true })

-- Mic mute (repeat)
hl.bind("XF86AudioMicMute", hl.dsp.exec_cmd(SCRIPT .. "/volume --toggle-mic"), { repeating = true })

-- Speaker mute (locked + repeat)
hl.bind("XF86AudioMute", hl.dsp.exec_cmd(SCRIPT .. "/volume --toggle"), { locked = true, repeating = true })
...
-- Volume toggles with modifiers
hl.bind(mainMod .. " + SHIFT + M", hl.dsp.exec_cmd(SCRIPT .. "/volume --toggle-mic"), { locked = true })
hl.bind(mainMod .. " + SHIFT + D", hl.dsp.exec_cmd(SCRIPT .. "/volume --toggle"))

-- Touchpad toggle (locked)
hl.bind("CTRL + " .. mainMod .. " + T", hl.dsp.exec_cmd(SCRIPT .. "/touchpad"), { locked = true })

-- Audio loopback
hl.bind("CTRL + ALT + M", hl.dsp.exec_cmd("pactl load-module module-loopback latency_msec=10"), { repeating = true })
hl.bind("CTRL + ALT + N", hl.dsp.exec_cmd("pactl unload-module module-loopback"),               { repeating = true })

-- ┌──────────────────────────────────────┐
-- │           Screenshots                │
-- └──────────────────────────────────────┘

local screenshotarea = [[hyprctl keyword animation "fadeOut,0,0,default"; grimblast --notify copysave area $(xdg-user-dir PICTURES)/Screenshots/$(date +'%d-%m-%Y+%H:%M:%S.png'); hyprctl keyword animation "fadeOut,1,4,default"]]

hl.bind(mainMod .. " + SHIFT + S", hl.dsp.exec_cmd(screenshotarea), { locked = true })
...
hl.bind(mainMod .. " + SHIFT + T", hl.dsp.exec_cmd(
    [[grimblast save area - | tesseract stdin stdout -l eng+vie | wl-copy]]
), { locked = true })
...
hl.bind("CTRL + ALT + Delete",           hl.dsp.exec_cmd("killall wlogout || wlogout --protocol layer-shell -b 5 -T 400 -B 400"), { repeating = true })
...
hl.bind("CTRL + " .. mainMod .. " + TAB",       hl.dsp.focus({ workspace = "e-1" }))
